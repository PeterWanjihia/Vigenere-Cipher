...
def text_indices(text, alphabet):
    return [alphabet.index(char) if char in alphabet else char for char in text]

def key_indices(key, text, alphabet):
    key = (key * (len(text) // len(key))) + key[:len(text) % len(key)]
    return [alphabet.index(char) if char in alphabet else char for char in key]

def V_encryption(p_indices, k_indices, alphabet):
    e_indices = [(p + k) % 26 if isinstance(p, int) else p for p, k in zip(p_indices, k_indices)]
    return [alphabet[index] if isinstance(index, int) else index for index in e_indices]
